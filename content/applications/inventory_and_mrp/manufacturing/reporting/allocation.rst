...
.. screenshot:: manufacturing-allocation-button
   :menu: Manufacturing app --> Operations --> Manufacturing Orders (open a confirmed MO)
   :shows: The top of a confirmed manufacturing order form, with the "Allocation" smart button
      (list icon) visible among the other smart buttons.
   :highlight: The "Allocation" smart button.
   :data: Demo company "YourCompany"; MO for a finished product with insufficient stock to cover an
      existing sales order.
   :module: mrp
   :notes: English UI, light theme, 1440px width, crop to the smart-button row.
...
.. screenshot:: manufacturing-allocation-reception-report-product
   :menu: Manufacturing app --> Operations --> Manufacturing Orders (open an MO) --> Allocation
      smart button
   :shows: The MRP Reception Report list for an MO producing a finished product, with one or more
      open delivery orders listed, each with an "Assign All" button and per-quantity "Assign"
      buttons.
   :highlight: The "Assign All" button of one delivery order line.
   :data: Demo company "YourCompany"; finished product "Rocking chair", two open delivery orders.
   :module: mrp
...
.. screenshot:: manufacturing-allocation-reception-report-component
   :menu: Manufacturing app --> Operations --> Manufacturing Orders (open a component MO) -->
      Allocation smart button
   :shows: The MRP Reception Report list for an MO producing a component, with one or more open
      MOs that require it listed, each with an "Assign All" button.
   :highlight: The "Assign All" button of one MO line.
   :data: Demo company "YourCompany"; component "Wood", two open "Rocking chair" MOs.
   :module: mrp
   :notes: English UI, light theme, 1440px width.
...
.. screenshot:: manufacturing-allocation-labels
   :menu: Manufacturing app --> Operations --> Manufacturing Orders (open an MO) --> Allocation
      smart button
   :shows: The generated PDF label sheet with one reservation label per assigned product.
   :highlight: One printed label showing the product and the order it is reserved for.
   :data: Demo company "YourCompany"; two assigned "Rocking chair" units.
   :module: mrp
   :notes: English UI, light theme, PDF preview, 1440px width.

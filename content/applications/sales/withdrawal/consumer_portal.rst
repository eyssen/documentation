...
`Who can withdraw`_). :guilabel:`Request Return` is the ordinary commercial return; withdrawal is the
legal no-reason cancellation.
...
opens the declaration through a **tokenized, login-free link** (order id + ``access_token`` in the
URL). This means even a guest who checked out without an account can exercise the right of
withdrawal directly from the e-mail.
...
The first step is a public page titled as the withdrawal declaration. It collects:
...
- **Items and quantities** — a table lists every eligible **product** line with its withdrawable
  quantity:
  ...
  - **Carrier shipping lines are not listed.** Delivery/shipping products (``is_delivery``) cannot
    be selected on the form and cannot create a shipping-only withdrawal. Refund of shipping fees is
    a commercial decision handled by the agent at refund time, not by withdrawing the delivery line.
...
#. sends the **durable-medium confirmation e-mail** to the address provided, listing the same
   products as the snapshot.
...
.. note::
   ...
   handled by the shop's return process (and by the agent with :guilabel:`Start Return / Refund` on
   the backend). The declaration itself does not change status to :guilabel:`Done`.
...
(RMA)`. The page has two sections:

- **ordinary return requests** (commercial RMA) at the top; and
- a separate :guilabel:`Elállások` ("Withdrawals") section below, where each confirmed declaration
  shows its reference, a green :guilabel:`Visszaigazolva` ("Confirmed") status and the submission
  date.

If both sections are empty, the page shows that there are no return requests yet.
